...
static const char* g_section = ".text";
static const char* g_lastSection = g_section;

void OpSection(const char* sec)
{
	g_section = sec;
}
...
void OpStart(int op, int sea, int tea, int op_changes_cycles, int supervisor_check)
{
  ...
  if(strcmp(g_section, g_lastSection)) {
	  ot("  .section %s\n", g_section);
	  g_lastSection = g_section;
  }
...
/*  int did_fetch=0;
  ...
  ot("\n");*/
	ot("  subs r5,r5,#%d ;@ Subtract cycles\n",Cycles);
	if(opend_check_interrupt) ot("  b CycloneNextInterrupt\n");
	else ot("  b CycloneNext\n");
	ot("\n");
	g_section = ".text";
}
