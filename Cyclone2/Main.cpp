...
static void PrintFramework()
{
  ...
  //BEN - changed
  ot("  bic r2, r8, #0x07\n");
  ot("  ldreq pc,[r6,r2,lsr #1] ;@ Jump to opcode handler\n");
...
//  ot("  tst r3,#0x20\n");
  ...
  //BEN - added
  ot("  bic r2, r8, #0x07\n");
  ot("  ldrge pc,[r6,r2,lsr #1] ;@ Jump to opcode handler\n");
  ot("  b CycloneEnd\n");
  ot("\n");
  ...
  ot("  ldrh r8,[r4],#2 ;@ Fetch next opcode\n");
  ot("  bic r0, r8, #0x07\n");
  ot("  ldrge pc,[r6,r0,lsr #1] ;@ Jump to opcode handler\n");
  ot("  b neoCpuEvent\n");
  ot("\n");

  ot("  .section .itcm\n");
  ot("CycloneNextInterrupt:\n");
  ot("  ldrh r8,[r4],#2 ;@ Fetch next opcode\n");
  ot("  blt neoCpuEvent\n");
  ot(";@ CheckInterrupt:\n");
  ot("  ldr r1,[r7,#0x44]\n");
  ...
  //BEN - added
  ot("  bic r2, r8, #0x07\n");
  ot("  ldreq pc,[r6,r2,lsr #1] ;@ Jump to next opcode handler\n");
  ot("  cmp r0,#6 ;@ irq>6 ?\n");
  ot("  andle r1,r1,#7 ;@ Get interrupt mask\n");
  ot("  cmple r0,r1 ;@ irq<=6: Is irq<=mask ?\n");
  ...
  //BEN - added
  ot("  bic r2, r8, #0x07\n");
  ot("  ldrle pc,[r6,r2,lsr #1] ;@ Jump to next opcode handler\n");
...
int MemHandler(int type,int size,int addrreg,int need_addrerr_check)
{
  ...
  if(type == 0 || type == 2) {
	  if(size == 0) ot("  bl neoCpuRead8 ;@ Call ");
	  else if(size == 1) ot("  bl neoCpuRead16 ;@ Call ");
	  else if(size == 2) ot("  bl neoCpuRead32 ;@ Call ");
  } else {
	  if(size == 0) ot("  bl neoCpuWrite8 ;@ Call ");
	  else if(size == 1) ot("  bl neoCpuWrite16 ;@ Call ");
	  else if(size == 2) ot("  bl neoCpuWrite32 ;@ Call ");
  }
...
static void PrintJumpTable()
{
  ...
  ot(ms?"  area |.data|, data\n":"  .data\n  .align 4\n\n");
    ...
    free(indexes);
#elif PACK_JUMPTABLE
  int blockTable[0x10000 / 8];
  int blockIndex = 0;

  ot("CycloneJumpTab%s\n", ms?"":":");

  for(int base = 0; base < 0x10000; base += 8) {
	  for(i = base; i < base + 8; i++) {
		  if(CyJump[i] != CyJump[base]) {
			  //block needs special jump table
			  ott("Tb%.4x",blockIndex," ;@ %.4x\n",base,base/8,2);
			  blockTable[blockIndex++] = base;
			  break;
		  }
	  }
	  if(i == base + 8) {
		  int op = CyJump[base];
		  //block doesn't need special table
  ...
  for(int block = 0; block < blockIndex; block++) {
	  int base = blockTable[block];
	  ot("Tb%.4x:\n", block);
	  //ot("  ldr r1, =Tb%.4xTable\n", block);
	  //ot("  add r1, pc, #4\n");
	  //ot("  and r0, r8, #0x7\n");
	  //ot("  ldr pc, [r1, r0, lsl #2]\n");
	  ot("  and r0, r8, #0x7\n");
	  ot("  ldr pc, [pc, r0, lsl #2]\n");
	  ot("  nop\n");
	  ot("Tb%.4xTable:\n", block);
	  for(i = base; i < base + 8; i++) {
		  int op = CyJump[i];
		  if(op>=0) ot(".long Op%.4x\n",op);
          else if(op==-2) ot(".long Op__al\n");
          else if(op==-3) ot(".long Op__fl\n");
          else ot(".long Op____\n");
	  }
	  ot("  .ltorg\n");
  }
...
static int CycloneMake()
{
  ...
  ot("  %s CycloneUnpack\n",globl);
  ot("  %s CycloneVer\n",globl);
  ot("  %s CycloneNextInterrupt\n",globl);
  ot("  %s CycloneEndNoBack\n",globl);
#if (CYCLONE_FOR_GENESIS == 2)
